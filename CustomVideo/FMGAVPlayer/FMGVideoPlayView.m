...
@property (nonatomic,weak) AVPlayer *current;
...
- (void)awakeFromNib
{
    self.playerLayer = [AVPlayerLayer playerLayerWithPlayer:self.createPlayer];
...
//    int duration = (int)CMTimeGetSeconds(self.player.currentItem.duration);
...
//    }
    ...
    if (isnan(duration)) {
        ...
        return [self stringWithCurrentTime:currentTime duration:0.0f];
    }
   // [self updateProgressInfo];
...
- (IBAction)sliderValueChange {
  ...
  //  [duration isKindOfClass:[NSString class]];
    ...
    if (isnan(duration)) {
        ...
        self.timeLabel.text = [self stringWithCurrentTime:currentTime duration:0.0f];
        return;
    }
